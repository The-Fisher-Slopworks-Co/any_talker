...
- **A user sees percentages, never token counts** (`ratelimit/share.ts`) — `/usage`
  in a DM (per window: share spent, then the reset on its own line — nothing
  else) and the Web App header (the same two figures as bars) both render a
  `UsageShare`, a type that carries
