...
- **A user sees percentages, never token counts** (`ratelimit/share.ts`) — `/usage`
  in a DM (a header, then two lines per window: the window with its share spent,
  and the time until reset) and the Web App header (the same two figures as
  bars) both render a `UsageShare`, a type that carries
