...
- **`/usage`** (DM only) — any user can ask where they stand: a progress bar per window with the
  share of that budget already spent and when it resets. The Web App shows the same two bars in a
  header above every screen. Both surfaces are **percentage-only by construction** — they are built
  from a type that carries no token counts at all, so the raw budget figures stay on the
