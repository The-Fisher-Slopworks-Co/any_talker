...
describe("gatherSpendOverview", () => {
  test("drops rows whose spend has aged out of every window", async () => {
    ...
    await storage.users.upsert(user("u1", "spender"));
    await storage.users.upsert(user("u2", "stale"));
    await storage.chats.upsert(chat("c1", "live"));
    await storage.chats.upsert(chat("c2", "quiet"));

    await storage.spend.addUser("u1", 0.5, NOW);
    await storage.spend.addChat("c1", 0.5, NOW);
    await storage.spend.addModel("vendor/live", 0.5, NOW);
    // Recorded, then aged past the 30-day "month" window: the directory rows
    // and the model set survive, the daily buckets don't.
    await storage.spend.addUser("u2", 0.5, NOW - 40 * DAY);
    await storage.spend.addChat("c2", 0.5, NOW - 40 * DAY);
    await storage.spend.addModel("vendor/stale", 0.5, NOW - 40 * DAY);
  ...
  test("drops rows too small to render as anything but $0.000000", async () => {
    const storage = new MemoryStorage();
    await storage.users.upsert(user("u1", "dust"));
    await storage.spend.addUser("u1", 1e-9, NOW);
    await storage.spend.addModel("vendor/dust", 1e-9, NOW);
  ...
  test("keeps a zero-row model that is flagged unpriced", async () => {
    const storage = new MemoryStorage();
    await storage.spend.addModel("vendor/unpriced", 0.5, NOW - 40 * DAY);
    await storage.spend.flagUnpriced("vendor/unpriced");
    await storage.spend.addModel("vendor/stale", 0.5, NOW - 40 * DAY);
