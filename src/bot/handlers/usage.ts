...
// that only the asker sees (`dispatchUsageCommand`); the handler itself does
// not care where it was asked.
...
// `explicit` says which of the two it was: the usage windows are family-wide,
// so `/usage` is a shared command, and only a bare one needs the who-acts-on-it
// gate (`handlesSharedCommand`) in a group holding several family bots.
...
export type UsageCommandOutcome = { kind: "usage"; text: string };
